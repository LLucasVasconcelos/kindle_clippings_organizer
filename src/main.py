...
def process_clippings():
    ...
    try:
        ...
        # Salvar anotações por livro
        print("Salvando anotações:\n")
        for book_title, book_clippings in sorted(grouped.items()):
            book_dir = os.path.join(output_dir, book_title)
            os.makedirs(book_dir, exist_ok=True)

            anotacoes_file = os.path.join(book_dir, "anotacoes.txt")
            if processor.export_to_text(book_clippings, anotacoes_file):
